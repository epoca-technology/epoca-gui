...
@Component({
  selector: 'app-discovery-payload-records-view',
  templateUrl: './discovery-payload-records-view.component.html',
  styleUrls: ['./discovery-payload-records-view.component.scss']
})
export class DiscoveryPayloadRecordsViewComponent implements OnInit, OnDestroy, IDiscoveryPayloadRecordsViewComponent {
	// Layout
	...
	ngOnInit(): void {
		...
		for (let r of this.items) {
			...
			successfulMean.push(r.discovery.successful_mean);
			...
			successfulPredictions: this._chart.getBarChartOptions(
				{
					series: [
						...
						{name:'Successful Mean', data: successfulMean}
					], 
					colors: [this._chart.upwardColor, this._chart.downwardColor, "#000000"],
				}, 
				this.ids, 
