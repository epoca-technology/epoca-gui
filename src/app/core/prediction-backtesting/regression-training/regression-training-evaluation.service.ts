...
export class RegressionTrainingEvaluationService implements IRegressionTrainingEvaluationService{
	...
	private readonly maxNeutrality: number = 95;
	...
	private isRegressionBroken(cert: IRegressionTrainingCertificate): {broken: boolean, state: string} {
		...
		const neutralPercent: number = <number>this._utils.calculatePercentageOutOfTotal(
			cert.regression_evaluation.neutral_predictions, 
			cert.regression_evaluation.neutral_predictions + cert.regression_evaluation.positions.length
		);
		if (neutralPercent > this.maxNeutrality) {
			broken = true;
			state = `Broken Regression: the neutrality is unacceptable as it represents ${neutralPercent}% of the predictions.`;
		}
	...
	private predictionNeutrality(
		...
		if (!broken) {
			...
			const change: number = <number>this._utils.calculatePercentageOutOfTotal(
				cert.regression_evaluation.neutral_predictions, 
				cert.regression_evaluation.neutral_predictions + cert.regression_evaluation.positions.length
			);
			...
			// Calculate the points
			let points: number = 0;
			if 		(change > 90) 	{ points = maxPoints/4 }
			else if (change >= 85) 	{ points = maxPoints/3 }
			else if (change >= 80) 	{ points = maxPoints/2.5 }
			else if (change >= 75) 	{ points = maxPoints/2 }
			else if (change >= 70) 	{ points = maxPoints/1.6 }
			else if (change >= 65) 	{ points = maxPoints/1.3 }
			else if (change >= 60) 	{ points = maxPoints/1.1 }
			else 					{ points = maxPoints }

			// Finally, return the evaluation
			return {
				points: points, 
				state: `The neutrality represents ${change}% of the predictions.`,
				state_class: this.getStateClass(points, maxPoints)
			}
		} else { return { points: 0, state: brokenState, state_class: "error"} }
	}
